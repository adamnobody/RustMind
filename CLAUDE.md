...
Core traits:
- **Structure-driven** — the user edits *structure* (parent/child links, sibling order), not pixel positions; the active layout strategy derives node positions from that structure (see the model section below). Exception: the `network` and `free` layouts keep free positions.
...
- **`positionMode: 'derived'`** (all strategies except `network`/`free`) — positions recomputed from structure after *every* structural change. `mindMapStore.recomputeIfDerived()` runs this and is called by add/delete/move/load. It's an invariant, not a feature.
- **`positionMode: 'stored'`** (`network`, `free`) — positions are soft, kept as-is (drag / force layout). Any `position` passed to `addChildNode` is the *final* placement here — for `derived` strategies it's just a placeholder `recomputeIfDerived()` immediately overwrites.
...
- **Edge shape is derived, never stored.** `edges/lib/resolveRoute.ts` is the single decision point, and every consumer (`MindEdge`, the collapse-button placement in `MindNode`, the layout previews) goes through it. Priority: (1) an explicit `data.style.routing` (see below) wins and survives layout switches; (2) otherwise `'auto'` → a tree edge takes the strategy's own `routeTreeEdge` if it declares one, a free edge never does; (3) fallback is the generic `routeEdge` by `edgeRouting`. Computed paths, buses, spines and anchors are recomputed from current node rects on every render and never touch `.rustmind`.
  - **`LayoutStrategy.routeTreeEdge?`** (`strategies/types.ts`) is how a layout owns its own connection semantics: it receives both endpoints, their **measured** rects, a `rectOf(id)` lookup and `nodes`/`edges`, and returns an SVG path + label position (`RoutedEdge`), or `null` to fall through. Declared by `hierarchy` (bottom→top), `right`/`left` (side→facing side), `both` (side picked per branch), `org` (shared sibling bus — same `busY` for every child of a parent, so the separate paths render as one bar), `logic` (local vertical bus at the parent's right edge), `timeline` (along the root-centre axis, then a vertical drop into the event) and `fishbone` (spine segment + diagonal rib to a per-category anchor). `tree`/`bubble`/`network` keep geometric port selection; `free` keeps literal stored handles.
- **Gotcha — edge handle vs. node position:** `sourceHandle`/`targetHandle` on a tree edge only drives the *rendered* connection point when the strategy's `edgeRouting === 'fixed'` (only `free`). Everywhere else the port comes from the strategy's `routeTreeEdge` (deterministic, semantic) or from `portToward`/`facingPorts` on the actual node rects — the stored handle is ignored for rendering. So requesting a specific handle side without also placing the node there only matters for the `free` layout.
- **Timeline and fishbone centre their root on the axis** (`position = -size/2`) because their routing finds the axis/spine as the horizontal through the root's *centre*. Other layouts write centre-math values straight into `position` (which ReactFlow reads as top-left) — a uniform half-node offset that routing is immune to, since it always works from real rects.
...
- **`layout/`** — the strategy system. `strategies/registry.ts` maps each `LayoutKind` (12: hierarchy/right/left/both/tree/org/logic/fishbone/timeline/bubble/network/free — see `engines/layoutTypes.ts`, default `hierarchy`) to a `LayoutStrategy` (`strategies/types.ts`): a pure `layout(nodes, edges)` positioner + `canConnect` predicate + `edgeConstraint`/`positionMode`/`edgeRouting` + an optional `routeTreeEdge` (see routing above). Add a layout type = add one strategy module + register it; shared route primitives (buses, axis branches) live in `strategies/routes.ts`. `applyLayout` is the wrapper the store calls. After any layout call, trigger `useUIStore.getState().triggerFitView()` in a short `setTimeout` so ReactFlow can measure nodes first. `LayoutTypeDialog` picks the type for new/existing docs; its previews (`components/layoutPreviews.tsx`) are not hand-drawn — they run a small fixed graph through the *real* strategy and `resolveEdgeRoute` and scale the result into a mini SVG, so a preview cannot drift from the canvas.
- **`canvas/`** — `MindMapCanvas` is the ReactFlow root. `useGlobalHotkeys` = node shortcuts (Tab=child, Enter=sibling, F2/printable=edit, Delete). Guards: skip if `editingNodeId !== null` or target is editable. Node **drag reparents structure** (XMind-style): `lib/dropTarget.ts` computes the drop target, `uiStore.dragIndicator` highlights it, drop calls `moveNode`. `isValidConnection` blocks illegal connects per the active strategy's `canConnect` (the store's `onConnect` is the second guard). `MiniMap` — `@xyflow/react`'s `<MiniMap>` doesn't render edges, so it's paired with a second `Panel` (own SVG overlay, same bottom-right position) that draws connection lines; the overlay's viewBox math must mirror the library's own bounds formula (node bounds ∪ viewport, `offsetScale`) or the lines drift off the nodes.
- **`nodes/`** — `MindNode` + `useNodeEditing` (inline textarea) + `useNodeActions`. `MindNodeData` (`nodes/types.ts`): `label, color?, isRoot?, note?, order?, status?, style?: NodeStyle, handleOffsets?`. `NodeStyle` (shape/border/font/colors) and `handleOffsets` store **only deviations from defaults** — `pruneStyle` strips fields equal to `DEFAULT_NODE_STYLE` (same helper the serializer uses).
  - **Status:** `data.status?: string` is a per-node id into `BUILTIN_STATUSES` (pending/in-progress/completed/failed, `nodes/types.ts`) or `projectSettings.customStatuses` (user-added, document-scoped). Set via `updateNodeData(id, { status })` — a plain field write, **never cascades to children**; the progress badge on an ancestor (`subtreeProgress` in `MindNode.tsx`) counts `status === 'completed'` only across leaf descendants, independent of any ancestor's own status.
  - **`NodeContextMenu`** — right-click menu (state: `uiStore.contextMenu`, session-only): create child on a specific handle side, change status (built-in/custom/"add custom" via `window.prompt`), delete, hide children. See the position/handle gotcha above before adding more directional actions.
- **`edges/`** — `MindEdge` renders paths; `lib/routing.ts` is pure geometry (ports + path builders), `lib/resolveRoute.ts` decides which one applies (see the routing priority above). `edges/types.ts`: `EdgeKind` (tree/free), `EdgeStyle`, `isTreeEdge`, `DEFAULT_TREE_EDGE_HANDLES`. Same prune-against-default contract as nodes.
  - **Two independent axes, don't conflate them:** `linePattern` (solid/dashed/dotted) is *stroke decoration*; `routing` (`auto`/`straight`/`bezier`/`smoothstep`/`orthogonal`/`step`, `EdgeRoutingChoice`) is *path geometry*, picked per edge in the Inspector. `smoothstep` and `step` share the mid-point polyline and differ by rounded vs. sharp corners; `orthogonal` uses the semantic port sides with a stubbed dogleg. Default is `auto`, which `pruneStyle` strips — so old files (no field) load as `auto` with no migration.
- **`inspector/`** — right-docked style panel. Edits exactly one element (single node OR single edge — `isEditableSelection` in uiStore); auto-opens on single selection unless the user hid it (`inspectorManuallyHidden` override). Once open, it **stays open** across selection changes (including deselecting to empty canvas) — only `hideInspector()` (the panel's own close button / toolbar toggle) closes it; `setSelection` never force-closes.
- **`persistence/`** — `fileService` (Tauri invoke wrappers, `.rustmind` files), `serializer` (`schema.ts` is `FILE_VERSION 4`), `usePersistence` (Save/SaveAs/Open/New with isDirty guards), `useWindowCloseGuard`, `recentFiles` (localStorage, for HomeScreen). `deserializeMindMap` → typed `LoadDocumentPayload`, migrates via `coerceLayoutKind` (unknown/legacy layoutType → `'hierarchy'`) and runs `normalizeStructure` (enforces one root, one parent per node, contiguous `order`). Old files without handles/`order`/`kind` are backfilled; legacy `checked: true` migrates to `status: 'completed'`.
...
- **`groups/`** — free-floating labeled regions (`GroupBox`) covering a set of node ids; bounds are computed on the fly from member node positions (`bounds.ts`), never stored. Groups are metadata, not structure — they don't participate in layout or `canConnect`.
- **`templates/`** — predefined starter mind maps (`templates.ts`: SWOT etc.), each with localized (RU/EN/DE/FR) labels built at pick-time via `buildTemplate`. `TemplateDialog` (driven by `uiStore.isTemplatePickerOpen`) turns a `TemplateDef` into a `LoadDocumentPayload` for `loadDocument`.

### App shell (`src/app/`)

Two screens toggled by local state in `App.tsx` (no router): `HomeScreen` (recent files + animated backdrop) → `EditorScreen`. Going home confirms unsaved changes and calls `resetDocument()`. `EditorScreen` calls `usePersistence()`, registers `useWindowCloseGuard()`, wraps content in `KeyboardProvider` (file hotkeys: Ctrl+S/Shift+S/O/N), and renders the toolbar, canvas, `Inspector`, `SettingsPanel`, `LayoutTypeDialog`, `TemplateDialog`.
