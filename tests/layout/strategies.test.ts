...
describe('отсутствие пересечений (базовый тест плотности) для Phase 2 раскладок', () => {
  ...
  it('fishbone: категории с под-причинами и глубокими цепочками не наслаиваются', () => {
    // Регрессия: раньше под-причины по глубине ложились на один Y и перекрывались
    // по X (сдвиг был меньше ширины узла). Строим 4 категории; у первой — цепочка
    // потомков в глубину, у второй — веер сиблингов.
    const nodes: AppNode[] = [makeNode('R', true)];
    const edges: AppEdge[] = [];
    const addChild = (parent: string, id: string, order: number) => {
      nodes.push({ ...makeNode(id), data: { label: id, order } });
      edges.push(treeEdge(parent, id));
    };
    // Категории корня.
    ['K0', 'K1', 'K2', 'K3'].forEach((k, i) => addChild('R', k, i));
    // K0 — глубокая цепочка (воспроизводит перекрывавшийся ряд из бага).
    addChild('K0', 'K0a', 0);
    addChild('K0a', 'K0b', 0);
    addChild('K0b', 'K0c', 0);
    // K1 — несколько сиблингов-под-причин.
    ['K1a', 'K1b', 'K1c'].forEach((s, i) => addChild('K1', s, i));
